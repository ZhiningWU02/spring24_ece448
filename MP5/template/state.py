...
class WordLadderState(AbstractState):
    ...
    def __lt__(self, other):
        ...
        fs, fo = (
            self.dist_from_start + self.h,
            other.dist_from_start + other.h,
        )
        if fs == fo:
...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        # Add neighbors in the move order (a,b,c,d) = (below, left, above, right)
        if zero_row < 2:
    ...
    def compute_heuristic(self):
        ...
        for tile in range(1, 9):
            cur_pos, goal_pos = find_pos(self.state, tile), find_pos(self.goal, tile)
            total += abs(cur_pos[0] - goal_pos[0]) + abs(cur_pos[1] - goal_pos[1])
        return total
    ...
    def __repr__(self):
        return "\n---\n" + "\n".join(
            [" ".join([str(r) for r in c]) for c in self.state]
        )
