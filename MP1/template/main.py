...
def main(args):
    ...
    print(
        "Please note that some of the arguments may be ignored depending on the data type and method"
    )
    ...
    #   - set get_features for linear, sine
    #   - set get_gradient for ik and gradient type for linear, sine
    #   - set value for A_init
    get_gradient = None
    if args.data_type in ["linear", "sine"]:
        if args.data_type == "linear":
            x, y = create_linear_data(
                args.num_samples, args.slope, args.intercept, args.x_range, args.noise
            )
        ...
        get_gradient = lambda c_: estimate_ik_gradient(
            ee_goal_loss, c_, num_samples=args.batch_size
        )
        # plot goal position
    ...
    # Analytical method only works for linear regression (not IK)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--noise", dest="noise", type=float, default=0.1, help="Noise in the data"
    )
    # GD and SGD args
    parser.add_argument(
        "--learning_rate",
    ...
    if args.num_iterations // args.plot_iterations > 100:
        print(
            "**Warning: You are plotting a lot of intermediate results, this may take a while**"
        )
    main(args)
