...
def create_linear_data(num_samples, slope, intercept, x_range=[-1.0, 1.0], noise=0.1):
    x_arr = np.random.rand(num_samples, 1) * (x_range[1] - x_range[0]) + x_range[0]
    y_arr = slope * x_arr + intercept + np.random.uniform(-noise, noise, x_arr.shape)
...
def mse_loss(y_pred, y_true):
    ...
    return np.sum(error**2) / y_pred.shape[0]
...
def get_linear_regression_gradient(A, X, y):
    gradient = 2 * X.T @ (X @ A - y) / y.shape[0]
    return gradient
...
def stochastic_gradient_descent(
    ...
    for _ in range(num_epochs):
        ...
        for i in range(0, data_size, batch_size):
            A = A - learning_rate * get_batch_gradient(A, indices[i : i + batch_size])
...
def create_sine_data(num_samples, x_range=[0.0, 2 * np.pi], noise=0.1):
    x_arr = np.random.rand(num_samples, 1) * (x_range[1] - x_range[0]) + x_range[0]
    y_arr = np.sin(x_arr) + np.random.uniform(-noise, noise, x_arr.shape)
...
def ik_loss(arm: Arm, config, goal):
    return np.linalg.norm(goal - arm.forward_kinematics(config)[-1])
...
def sample_near(num_samples, config, epsilon=0.1):
    noise = np.random.uniform(-epsilon, epsilon, (num_samples, config.shape[0]))
...
def estimate_ik_gradient(loss, config, num_samples):
    ...
    gradient = np.zeros(config.shape)
    max_loss = loss(config)
    for i in range(num_samples):
        if max_loss < loss(samples[i]):
            max_loss = loss(samples[i])
            gradient = samples[i] - config
