...
def test_create_linear_data(
    num_samples=100, slope=2, intercept=1, x_range=[-10, 10], noise=0
):
    ...
    for i in range(num_samples):
        assert (
            ...
            and y[i] >= slope * x[i] + intercept - noise
        )

    # check that the samples are random
    x_new, _ = create_linear_data(
        num_samples=num_samples,
        slope=slope,
        intercept=intercept,
        x_range=x_range,
...
def test_analytical_linear_regression():
    ...
    x, y = create_linear_data(
        num_samples=100, slope=-3, intercept=2, x_range=[0, 5], noise=0.1
    )
    X = get_simple_linear_features(x)
    A = analytical_linear_regression(X, y)
...
def test_linear_regression_gradient():
    ...
    x, y = create_linear_data(
        num_samples=100, slope=-10, intercept=5, x_range=[0, 5], noise=0.1
    )
    X = get_simple_linear_features(x)
    grad = get_linear_regression_gradient(np.array([[-10], [5]]), X, y)
    assert (
        np.linalg.norm(grad) < 0.2
    )  # 0.2 is arbitrary... just to make sure it's close enough
...
def test_gradient_descent():
    ...
    x, y = create_linear_data(
        num_samples=100, slope=-10, intercept=5, x_range=[0, 5], noise=0.5
    )
    X = get_simple_linear_features(x)
    A_init = np.array([[0], [0]])
    get_gradient = lambda A: get_linear_regression_gradient(A, X, y)
...
def test_stochastic_gradient_descent():
    ...
    x, y = create_linear_data(
        num_samples=100, slope=2, intercept=1, x_range=[-10, 10], noise=0.1
    )
    X = get_simple_linear_features(x)
    A_init = np.array([[0], [0]])
    get_batch_gradient = lambda A, indices: get_linear_regression_gradient(
        A, X[indices], y[indices]
    )
    A1 = stochastic_gradient_descent(
        get_batch_gradient,
        A_init,
        learning_rate=0.01,
        num_epochs=1,
    ...
    A2 = stochastic_gradient_descent(
        get_batch_gradient,
        A_init,
        learning_rate=0.01,
        num_epochs=1,
    ...
    A1 = gradient_descent(
        lambda A: get_linear_regression_gradient(A, X, y),
        A_init,
        learning_rate=0.01,
        num_iterations=100,
    )
    ...
    assert compute_model_error(
        x, y, A1, get_simple_linear_features
    ) > compute_model_error(x, y, A2, get_simple_linear_features)
...
def test_get_polynomial_features(num_data, num_features, degree):
    ...
    for i in range(degree + 1):
        assert np.all(
            np.isclose(
                X[:, i * num_features : (i + 1) * num_features], x ** (degree - i)
            )
        )
