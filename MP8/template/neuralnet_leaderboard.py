...
class NeuralNet(nn.Module):
    def __init__(self, lrate, loss_fn, in_size, out_size):
        ...
        self.optimizer = optim.AdamW(self.parameters(), lr=lrate, weight_decay=5e-5)
...
def fit(train_set, train_labels, dev_set, epochs, batch_size=100):
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(net.optimizer, T_max=epochs)
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if epoch > 0 and np.random.random() > 0.5:
                x_batch = x_batch.view(x_batch.shape[0], 3, 31, 31)
                x_batch = torch.flip(x_batch, dims=[3])
    ...
    with torch.no_grad():
        dev_outputs = net(dev_set)
        preds = torch.argmax(dev_outputs, dim=1)
    ...
    return losses, preds.cpu().numpy().astype(np.int64), net
